...
public class RoomService
{
    ...
    public (GameRoom room, string? toast) ExecuteMove(GameRoom room, Player pCtrl, Piece piece, MoveDestination target, List<int> dieIndicesUsed)
    {
        ...
        // Use calculateDestination-compatible check that accounts for blocking/move validation
        if (rewardTurn)
        {
            ...
            for (int d = 1; d <= 6; d++)
            {
                ...
                {
                    if (pc.State == PieceState.Home || pc.HasKilledThisTurn) continue;
                    var dest = GameLogic.CalcDestSimple(pCtrl, pc, d, room.Players, room.Settings, [d]);
                    if (dest != null)
                    {
                        // Additional validation: ensure dest isn't blocked or hitting a safe-zone block
                        var sz = GameLogic.EffectiveSafeZones(room.Settings);
                        if (dest.TargetState == PieceState.Board && !sz.Contains(dest.TargetPos) &&
                            GameLogic.HasOpponent(dest.TargetPos, pCtrl.Id, room.Players))
                        {
                            continue; // Would hit opponent - only allow if no other move exists
                        }
                        hasMovable = true;
                        break;
                    }
        ...
        if (room.Settings.DiceCount == 1 && piece.HasKilledThisTurn)
            rewardTurn = true;

        if (room.Settings.BlostOnDoubleFour && pCtrl.HasKilled &&
            pCtrl.Pieces.Any(p => p.State == PieceState.Board))
        {
             safeZones = GameLogic.EffectiveSafeZones(room.Settings);
            foreach (var op in room.Players)
            {
                if (op.Id == pCtrl.Id || op.Id == pCtrl.PartnerId || !op.IsActive) continue;
                foreach (var opc in op.Pieces.Where(x => x.State == PieceState.Board && !safeZones.Contains(x.Pos)).ToList())
                {
                    op.TimesHit++;
                    if (room.Settings.PrisonRule) { opc.State = PieceState.Prison; opc.Pos = -2; opc.PrisonerOf = pCtrl.Id; }
                    else { opc.State = PieceState.Yard; opc.Pos = -1; opc.PrisonerOf = null; }
    ...
    public void EndTurn(GameRoom room)
    {
        ...
        bool skipExtra = room.Settings.SkipOnDoubleThree &&
                         room.LastDiceResults.Count >= 2 &&
                         room.LastDiceResults[0] == 3 && room.LastDiceResults[1] == 3;
        ...
        do
        {
            room.TurnSlot = (room.TurnSlot + 1) % 4;
            safety++;
            if (safety > 8) { room.Phase = GamePhase.End; return; }
        }
        while (!room.Players[room.TurnSlot].IsActive ||
               (room.Players[room.TurnSlot].Finished && !room.Settings.TeamPlay) ||
               !GameLogic.HasAnyPossibleMove(room.Players[room.TurnSlot], room.Players, room.Settings));
